...
function makeMcpStreamFetch(
  ...
  return ((url: string, opts: MockRequestInit) => {
    const u = String(url);
...
test("mcp call sends JSON-RPC initialize then tools/call", async () => {
  ...
  globalThis.fetch = ((url: string, opts: MockRequestInit) => {
    const u = String(url);
    const body = opts?.body ? JSON.parse(opts.body) : null;
  ...
  try {
    ...
    assert.equal((calls[0].body as LooseDeep).method, "initialize");
    assert.equal((calls[1].body as LooseDeep).method, "tools/call");
    assert.equal((calls[1].body as LooseDeep).params.name, "omniroute_get_health");
    assert.deepEqual((calls[1].body as LooseDeep).params.arguments, {});
  } finally {
...
test("mcp call passes session-id header on tools/call", async () => {
  ...
  const origFetch = globalThis.fetch;
  globalThis.fetch = ((_url: string, opts: MockRequestInit) => {
    const body = opts?.body ? JSON.parse(opts.body) : null;
    if (body && body.method === "initialize") {
      return Promise.resolve(
        makeResp(
