...
import type { PoolAllocation } from "../../src/lib/db/quotaPools.ts";
...
async function resetStorage() {
  ...
  if (typeof (apiKeysDb as LooseDeep).resetApiKeyState === "function") {
    (apiKeysDb as LooseDeep).resetApiKeyState as () => void;
...
async function mkConn(provider: string, name: string): Promise<string> {
  ...
  return (conn as LooseDeep).id as string;
}

/** Get allocations for a pool (via getPool). */
function getAllocs(poolId: string): PoolAllocation[] {
