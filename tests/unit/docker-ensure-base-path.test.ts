...
test("ensureDockerBasePath is a no-op when runtime matches the baked sentinel", () => {
  ...
  const result = ensureDockerBasePath({
    appRoot,
    env: { OMNIROUTE_BASE_PATH: "" } as NodeJS.ProcessEnv,
  });
...
test("ensureDockerBasePath patches a root image when a subpath is configured", () => {
  ...
  const result = ensureDockerBasePath({
    appRoot,
    env: { OMNIROUTE_BASE_PATH: "/omniroute" } as NodeJS.ProcessEnv,
  });
  assert.equal(result.action, "patched");
  assert.equal(
    fs.readFileSync(path.join(appRoot, "BUILD_OMNIROUTE_BASE_PATH"), "utf8"),
    "/omniroute\n"
  );
...
test("ensureDockerBasePath rejects unset runtime on subpath images", () => {
  ...
  assert.throws(
    () => ensureDockerBasePath({ appRoot, env: {} as NodeJS.ProcessEnv }),
    /built for subpath/
  );
});
