...
tests/b.py ....................
============ 142 passed in 3.21s ============`;
  const r = renderTestGreen(input, det("test-pytest") as unknown as CommandDetectionResult);
...
test("ANSI-colored FAIL with no numeric failed-count ⇒ no-op (regression: \\bFAIL\\b defeated by ANSI)", () => {
  ...
  const r = renderTestGreen(input, det("test-jest") as unknown as CommandDetectionResult);
  assert.equal(r.changed, false);
});
