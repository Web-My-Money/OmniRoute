...
test("defaults to 100 MB when env is unset (#4719)", () => {
  assert.equal(resolveMaxUploadSizeBytes({} as NodeJS.ProcessEnv), 100 * MB);
});

test("honors OMNIROUTE_DB_IMPORT_MAX_MB (#4719)", () => {
  assert.equal(
    resolveMaxUploadSizeBytes({ OMNIROUTE_DB_IMPORT_MAX_MB: "256" } as NodeJS.ProcessEnv),
    256 * MB
  );
});
...
test("clamps absurd values to the 4 GB ceiling (#4719)", () => {
  assert.equal(
    resolveMaxUploadSizeBytes({ OMNIROUTE_DB_IMPORT_MAX_MB: "999999" } as NodeJS.ProcessEnv),
    4096 * MB
  );
});
...
test("falls back to default for invalid / out-of-range values (#4719)", () => {
  assert.equal(
    resolveMaxUploadSizeBytes({ OMNIROUTE_DB_IMPORT_MAX_MB: "abc" } as NodeJS.ProcessEnv),
    100 * MB
  );
  assert.equal(
    resolveMaxUploadSizeBytes({ OMNIROUTE_DB_IMPORT_MAX_MB: "0" } as NodeJS.ProcessEnv),
    100 * MB
  );
  assert.equal(
    resolveMaxUploadSizeBytes({ OMNIROUTE_DB_IMPORT_MAX_MB: "-5" } as NodeJS.ProcessEnv),
    100 * MB
  );
});
...
test("floors fractional MB values (#4719)", () => {
  assert.equal(
    resolveMaxUploadSizeBytes({ OMNIROUTE_DB_IMPORT_MAX_MB: "150.9" } as NodeJS.ProcessEnv),
    150 * MB
  );
});
