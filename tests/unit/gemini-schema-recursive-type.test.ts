...
test("#9268 injects type:object on nested properties without type", () => {
  ...
  const result = cleanJSONSchemaForAntigravity(input) as LooseDeep;
  const props = result.properties as LooseDeep;
  const address = props.address as LooseDeep;
...
test("#9268 injects type:object on nested items array schemas", () => {
  ...
  const result = cleanJSONSchemaForAntigravity(input) as LooseDeep;
  const props = result.properties as LooseDeep;
  const items = props.items as LooseDeep;
  const inner = items.items as LooseDeep;
...
test("#9268 injects type:object on deeply nested schemas (3+ levels)", () => {
  ...
  const result = cleanJSONSchemaForAntigravity(input) as LooseDeep;
  const l1 = (result.properties as LooseDeep).level1 as LooseDeep;
  const l2 = (l1.properties as LooseDeep).level2 as LooseDeep;
  const l3 = (l2.properties as LooseDeep).level3 as LooseDeep;
...
test("#9268 schema already typed is not double-injected", () => {
  ...
  const result = cleanJSONSchemaForAntigravity(input) as LooseDeep;
  const nested = (result.properties as LooseDeep).nested as LooseDeep;

  assert.equal(nested.type, "object", "already-typed nested must keep its type");
  // Ensure properties is not clobbered
  const nestedProps = nested.properties as LooseDeep;
...
test("#9268 node with required but no properties still gets type:object", () => {
  ...
  const result = cleanJSONSchemaForAntigravity(input) as LooseDeep;
  const ref = (result.properties as LooseDeep).ref as LooseDeep;
