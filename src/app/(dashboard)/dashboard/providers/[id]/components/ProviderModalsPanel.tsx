...
interface ProviderModalsPanelProps {
  ...
  externalLinkCopy: (text: string, key?: string) => Promise<boolean>;
  // Edit connection
  ...
  return (
    <>
      {showRiskNoticeModal && subscriptionRisk && (
        <RiskNoticeModal
          variant={(providerInfo.riskNoticeVariant as RiskNoticeVariant) ?? "oauth"}
      ...
      {providerId === "codex" && externalLinkModalOpen && (
        <ExternalLinkModal
          ...
          url={externalLinkUrl}
          copied={externalLinkCopied || false}
